...
| “No mocks or placeholders remain in the critical path” | **Conditional** | Production paths do not silently fall back to mocks. Deterministic browser tests do substitute explicit Google/HTTP boundaries, and real Google, semantic-model, WASIX, WebContainer, and Chutes behavior require separate live gates. Therefore the stronger claim is not accepted until those provider gates pass in the release environment. |
...
| Bash shell in the browser | **Unavailable** | The WASIX pack has a pinned, content-addressed research candidate with bounded mounts/output, worker isolation, and tracked hard cancellation in [wasix-pack.ts](../src/execution/wasix-pack.ts) and [wasix-worker.ts](../src/execution/wasix-worker.ts). The live Chromium gate is a **NO-GO**: raw SDK status remained `45`, the control wrapper did not faithfully propagate `false`/`exit 7`, and the mounted known-input run surfaced neither separated output nor adopted mutation. No run proved faithful child status, separated output, bidirectional workspace writeback, and cancellation together, so [runtime-registry.ts](../src/execution/runtime-registry.ts) remains fail-closed at `unavailable`. |
...
1. Resolve the current WASIX **NO-GO**, then promote browser Bash only after one
   live run proves child exit status, stdout/stderr, bidirectional workspace
   mutation/adoption, and cancellation together.
