...
**Correction (as built):** The contrast half of this acceptance was first discharged for `--v-failed` and `--copper` by granting them WCAG 1.4.11's 3:1 and enumerating, in a frozen ledger, the twenty `color:` rules that contradicted the premise — sixteen of them a word at caption size on a 4.24–4.29:1 bed. That was an exemption, not the acceptance. Both tokens were then raised in `src/ui/tokens.css` (`--v-failed` #c86758 → #ce7769, `--copper` #b8734f → #be805f), so the criterion is now enforced literally: `src/ui/css-variable-contract.test.ts` holds each of the six named tokens to 4.5:1 on **both** the effective `--surface` and `--surface-raised` for every theme × mode, and the worst case — Verdigris' `--surface-raised` #1b292a — measures 4.63:1 and 4.61:1. The raise also retired `routes.css`'s hand-written `#e4877e` for the invalid audit state and lifted every `--danger`/`--signal-red` alias with it. The ledger survives as the record of which rules depend on that floor, not as open debt.
