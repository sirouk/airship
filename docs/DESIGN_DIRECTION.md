...
4. **No new runtime dependency.** `AIRSHIP_DESIGN_BLUEPRINT.md:2250` pins runtime deps at exactly the set `package.json` already lists; `sigma` and `graphology` are not in it, and the memory graph ships as a hand-written 2D canvas instead. No popover library, no headless-UI, no CSS-in-JS, no icon package. `<Popover>` is hand-rolled on the existing `focus-trap.ts`.
