...
**Fix.** In src/ui/app.tsx at the transcript container (~line 4339), change the static `class="transcript"` to a computed class that appends ` no-turns` when `messages.length <= 1` — the same predicate already used at line 4391 to gate `.transcript-starters` — e.g. `class={`transcript${messages.length <= 1 ? " no-turns" : ""}`}`.

In src/ui/styles.css, after the `.transcript` rule at line 1160, add:
...
**Fix.** Extend src/ui/chat/markdown.tsx, keeping createElement-only rendering and MARKDOWN_LIMITS bounds. Five coordinated edits the original fix under-specified:
...
Extend src/ui/chat/markdown.test.ts (currently 31 lines) with cases for each, including the `**bold**`-vs-`*italic*` ordering, the snake_case non-italicization guard, `---` immediately after a prose line, and nested-depth preservation.
