...
| Node/npm projects through WebContainer | **Conditional** | [live-webcontainer.spec.ts](../e2e/live-webcontainer.spec.ts) starts from one `web-baseline` conversation, performs a real npm readiness probe, installs pinned Vite, then builds it in a second command using the same page-local dependency state—without a fork, reload, or hidden backend. It revision-checks the lockfile and built assets back into Workspace. [agent-runtime-promotion.test.ts](../src/core/agent-runtime-promotion.test.ts) separately proves the real agent loop and journal keep activation, install, build, and final response in one turn; [master-browser-acceptance.spec.ts](../e2e/master-browser-acceptance.spec.ts) proves Node writeback appears in Editor/SCM. Availability depends on a supported browser, isolation headers, registry delivery, and the external WebContainer runtime/terms. |
...
| Ephemeral is the default starting Vault mode, with durable providers explicit | **Implemented** | [platform-shell.tsx](../src/ui/platform-shell.tsx) resolves an unconfigured build to Ephemeral page memory, and [vault-view.tsx](../src/ui/vault-view.tsx) puts Ephemeral before Local Device in the provider selector and comparison table. Choosing Local Device remains an explicit route into the encrypted offline Vault ceremony. |
| Google Drive is available as the configured default Vault provider | **Implemented** | A build with a valid `VITE_GOOGLE_CLIENT_ID` may explicitly select Drive at build time. [google-drive-object-store.ts](../src/storage/google-drive-object-store.ts) implements the shared object-store interface, workspace naming, range reads, CAS, and resumable upload recovery. Unconfigured builds do not offer Drive as an implicit default. |
| The Drive default only ships where the build is configured | **Implemented** | `VITE_GOOGLE_CLIENT_ID` is inlined at build time, so an unconfigured build has no Drive connect branch left in the bundle. [pages.yml](../.github/workflows/pages.yml) forwards `vars.VITE_GOOGLE_CLIENT_ID` and selects `google-drive` only when that variable is non-empty, otherwise `ephemeral`. [google-drive-auth.ts](../src/storage/google-drive-auth.ts) exports `isDeployableGoogleOAuthClientId`, the same predicate the authorizer enforces at construction. Existing explicit preferences remain intact; only an unavailable persisted Drive choice is revalidated. |
| Runtime reclamation of provider-side probe litter | **Implemented, deterministic; live confirmation still rides with the Drive live gates** | The aged-supersession queue in [reclamation-queue.ts](../src/vault/reclamation-queue.ts) is one encrypted, CAS-guarded durable object per Vault; [encrypted-workspace.ts](../src/vault/encrypted-workspace.ts) and [context-fabric-port.ts](../src/vault/context-fabric-port.ts) record every supersession at commit time. [coordinator.ts](../src/vault/coordinator.ts) `runReclamationSweep` ages candidates (default seven days, one-hour floor, ninety-day ceiling), re-verifies each against the freshest committed manifest or routing mirror, reclaims through `ReclaimableObjectStore.trash` in bounded batches, reconciles still-referenced entries out of the queue, and commits exactly which removals the provider confirmed ([reclamation.ts](../src/vault/reclamation.ts)). [google-drive-object-store.ts](../src/storage/google-drive-object-store.ts) additionally implements `UntrackedObjectSweepStore`, paging the segments folder for bodies no index entry names and trashing them by provider identifier with a fresh-index recheck per call. [vault-view.tsx](../src/ui/vault-view.tsx) exposes the run as an explicit **Reclaim storage** action with a receipt-driven status sentence. Providers without reclamation capability get an honest "cannot reclaim" note; journal segments from a lost append CAS remain retry-recoverable residue; Local Device has no sweep wiring yet. Probe-litter and purge behavior are unchanged. |
