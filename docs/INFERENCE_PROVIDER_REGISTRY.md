...
| --- | --- | --- | --- | --- |
| Chutes | Yes when the deployment supplies a reviewed Browser/native registration with Authorization Code, S256 PKCE, and token endpoint authentication `none`; localhost uses the same direct flow. The confidential development handler is a protocol-test harness, not a browser-client or extension feature. | Chutes API key or the reviewed OAuth access token | Chutes is the reviewed direct E2EE path. Existing Chutes transport and attestation behavior is unchanged. | Existing Chutes E2EE v1 |
