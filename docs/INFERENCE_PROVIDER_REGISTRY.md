...
| --- | --- | --- | --- | --- |
| Chutes | Yes. Hosted static builds require a reviewed Browser/native registration with Authorization Code, S256 PKCE, and token endpoint authentication `none`. The current localhost registration is confidential and uses the same-origin process-held token handler; the extension never carries its secret. | Chutes API key or the reviewed OAuth access token | Chutes is the reviewed direct E2EE inference path. OAuth token handling and inference transport are separate boundaries. | Existing Chutes E2EE v1 |
