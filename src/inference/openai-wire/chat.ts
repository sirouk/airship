...
export class OpenAiStreamAssembler {
  ...
  consume(data: string): InferenceEvent[] {
    if (this.finalized) {
      throw new OpenAiWireError("invalid-response", "Received model data after stream finalization.");
    }

    const parsed = parseJson(data, "OpenAI stream event");
    if (!isRecord(parsed)) {
      throw new OpenAiWireError("invalid-response", "OpenAI stream event must be a JSON object.");
    }
  ...
  finalize(): { toolCalls: ToolCall[]; finishReason: "stop" | "tool-calls" | "length" } {
    if (this.finalized) {
      throw new OpenAiWireError("invalid-response", "OpenAI stream was finalized more than once.");
    }
    this.finalized = true;
  ...
  private consumeToolDeltas(rawCalls: unknown[]) {
    for (const rawCall of rawCalls) {
      if (!isRecord(rawCall) || !Number.isSafeInteger(rawCall.index) || (rawCall.index as number) < 0) {
        throw new OpenAiWireError("tool-call-invalid", "OpenAI tool-call delta has an invalid index.");
      }
      ...
      if (rawCall.type !== undefined && rawCall.type !== "function") {
        throw new OpenAiWireError("tool-call-invalid", "OpenAI returned a non-function tool call.");
      }
...
function parseJson(
  value: string,
  label: string,
  code: "invalid-response" | "tool-call-invalid" = "invalid-response",
): unknown {
  try {
    return JSON.parse(value);
  } catch (error) {
    throw new OpenAiWireError(code, `${label} is not valid JSON.`, { cause: error });
  }
}
