...
async function bootAirship(): Promise<void> {
  if (import.meta.env.PROD) {
    const { establishControlledNavigationBoundary } = await import("./controlled-navigation");
    const ready = await establishControlledNavigationBoundary({
      basePath: AIRSHIP_BASE_PATH,
      revision: new URL(import.meta.url).pathname.split("/").pop() || "build",
      serviceWorkerPath: AIRSHIP_SERVICE_WORKER_PATH,
    });
    if (!ready) return;
  }
