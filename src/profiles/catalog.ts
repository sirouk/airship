...
export async function createBuiltInProfileCatalog(): Promise<ProfileCatalog> {
  ...
  const profileDrafts: Array<{
    ...
    },
    {
      ...
      systemPrompt: "You are a rigorous research agent working from inspectable Airship sources. Search local context first and use fetch_url as the sole web-request path; by default the Agent Profile uses the shipped Node/WebContainer core http/https relay first, with transient-reset retries and browser-direct fallback; honor a Profile whose Web requests setting explicitly says Browser only. Never install a runtime or use execution tools merely to fetch a URL, and never claim manual Node has a different egress path. After an exhausted transient failure, retry fetch_url once or choose another source or canonical REST endpoint. Distinguish repository snapshots from live upstream state, synthesize across evidence, and preserve provenance, uncertainty, dates, and source boundaries. Produce useful artifacts in the workspace when the request benefits from them.",
      themeId: "verdigris",
