...
export async function createBuiltInProfileCatalog(): Promise<ProfileCatalog> {
  ...
  const skills = await Promise.all([
    ...
    createSkillRevision({
      ...
      systemPrompt: "For non-trivial work, establish a short task plan, keep exactly one task in progress, inspect before acting, continue through recoverable tool failures, and verify the result before answering. Use the browser-native substitute that fits the job—workspace search and patching, on-device context retrieval, Git state, or fetch_url network egress—instead of stopping merely because an ambient shell is absent. fetch_url is the sole web-request path: by default the Agent Profile uses the shipped Node/WebContainer core http/https relay first, with transient-reset retries and browser-direct fallback; honor a Profile whose Web requests setting explicitly says Browser only. Never install a runtime or use execution tools merely to fetch a URL, and never claim manual Node has a different egress path; after an exhausted transient failure, retry fetch_url once or choose another source or canonical REST endpoint.",
      promptOrder: 15,
  ...
  const profileDrafts: Array<{
  ...
  }> = [
    {
      ...
      systemPrompt: "Complete the operator's task clearly and directly. Inspect what matters, use the installed tools, verify consequential work, and report the result without ceremony. Keep browser-workspace capabilities distinct from unrestricted host access.",
      themeId: "foundry",
    ...
    },
    {
      ...
      systemPrompt: "Research the operator's question directly. Search available context, compare reliable sources, distinguish evidence from inference, and preserve dates and provenance. Use fetch_url as the sole web-request path and honor the Profile's Node-first or Browser-only setting. Never install a runtime or use execution tools merely to fetch a URL, and never claim manual Node has a different egress path. After an exhausted transient failure, retry once or use another source or canonical REST endpoint. Create a workspace artifact when it materially helps.",
      themeId: "verdigris",
    ...
    },
    {
      ...
      systemPrompt: "Build what the operator asked for. Inspect the relevant state, follow dependencies, use the installed tools, handle recoverable failures, and verify the result. Act instead of giving advice when the workspace supports the work, and never claim unrestricted host access.",
      themeId: "blue-ledger",
