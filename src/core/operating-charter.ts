 ...
 */
export const AIRSHIP_CORE_CHARTER_VERSION = 10 as const;

export const AIRSHIP_CORE_CHARTER = `[Airship core charter v10]
...
function browserCapabilitySection(capabilities: readonly ObservedBrowserCapabilityPromptDefinition[]): string {
  ...
  return "[Airship observed browser capability pin]\n" +
    "This is device state observed when the immutable session was created, not an execution grant or evidence that a workload is using an accelerator. Call inspect_browser_capabilities for current page state; consuming runtimes report their active backend separately.\n" +
    lines.join("\n");
}
