...
| 14 | manifest assembly | `createSession` → airship `createSessionManifest` | byte-identical manifests (verified against a direct call with pinned `now`) |
| 15 | runtime gate (load-agent-runtime seam) | `runPrimeTurn`, `sessionRuntimeKind` | evidence-based custody: the session writes a one-shot `prime.notice` custody marker at bootstrap so the kind evidence exists for every prime session (see deviations) |
...
- **`inference.started.posture`** = transport posture, else manifest
  ...
  evidence the runtime cannot see.
...
- **Custody marker**: one `prime.notice` at session bootstrap so the runtime
  gate's evidence classification (`sessionRuntimeKind`) is honest for plain
  prime turns (no kernel events exist to classify otherwise).
