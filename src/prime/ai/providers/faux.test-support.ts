 ...
 * in-memory provider for tests. Stream event semantics match the real
 * providers exactly (start -> per-block start/delta/end -> one terminal
 * event, aborts as values), so test harnesses can exercise stream consumers
 * without network, keys, or clocks.
 */
...
function assistantContentToText(content: (TextContent | ThinkingContent | ToolCall)[]): string {
  return content
    .map((block) => {
      if (block.type === "text") {
        return block.text;
