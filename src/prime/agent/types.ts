/**
 * Port of prime-agent packages/agent/src/types.ts.
 *
 * The hook contracts, event vocabulary, and state surface are preserved
 * exactly. What changes: tool parameter schemas are plain JSON Schema objects
 * (no typebox runtime dependency), so the agent-side generic on AgentTool
 * binds the argument *value* type directly instead of typebox's Static<>.
 */
 ...
 * thinking-level metadata from ../ai to detect support for a concrete model.
 */
...
export interface AgentTool<TArguments = unknown, TDetails = any> extends Tool {
  /** Human-readable label for UI display. */
  label: string;
  /**
   * Optional compatibility shim for raw tool-call arguments before schema validation.
   * Must return an object that matches the tool's argument schema.
   */
  prepareArguments?: (args: unknown) => TArguments;
