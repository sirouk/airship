...
import { agentLoop, agentLoopContinue, runAgentLoop } from "./agent-loop";
import {
  ...
  identityConverter,
  MockAssistantStream,
  ThrowingResultStream,
} from "./test-utils/fixtures";
import type { AgentContext, AgentEvent, AgentLoopConfig, AgentMessage, AgentTool } from "./types";

/**
 * Port of prime-agent packages/agent/test/agent-loop.test.ts.
 * TypeBox schemas become plain JSON Schema; everything else is scenario-faithful.
 */
...
describe("agentLoop with AgentMessage", () => {
  ...
  it("should not wait for a pending terminal result after abort", async () => {
    ...
    const controller = new AbortController();
    const finalMessage = createAssistantMessage([{ type: "text", text: "complete" }]);
    let resolveResult: ((message: typeof finalMessage) => void) | undefined;
    const pendingResult = new Promise<typeof finalMessage>((resolve) => {
  ...
  it("should preserve a successful tool result when abort fires during update flush", async () => {
    const controller = new AbortController();
    const tool: AgentTool<Record<string, never>, Record<string, never>> = {
      name: "work",
      label: "Work",
      description: "Work",
      parameters: { type: "object", properties: {} },
  ...
  it("should execute mutated beforeToolCall args without revalidation", async () => {
    ...
    const tool: AgentTool<{ value: string }, { value: string | number }> = {
      name: "echo",
      label: "Echo",
  ...
  it("should prepare tool arguments for validation", async () => {
    ...
    const tool: AgentTool<{ edits: Array<{ oldText: string; newText: string }> }, { count: number }> = {
      name: "edit",
      label: "Edit",
  ...
  it("should force sequential execution when a tool has executionMode=sequential even with default parallel config", async () => {
    ...
    const slowTool: AgentTool<{ value: string }, { value: string }> = {
      name: "slow",
      label: "Slow",
  ...
  it("should force sequential execution when one of multiple tools has executionMode=sequential", async () => {
    ...
    const fastTool: AgentTool<{ value: string }, { value: string }> = {
      name: "fast",
      label: "Fast",
...
describe("agentLoop tool pipeline error encoding", () => {
  /** Stream factory: first call requests the given tool calls, later calls finish with text. */
  function toolCallThenDone(assistant: ReturnType<typeof createAssistantMessage>, doneText = "done") {
    let callIndex = 0;
    return () => {
      const stream = new MockAssistantStream();
      queueMicrotask(() => {
        if (callIndex === 0) {
          ...
          stream.push({
            type: "done",
            reason: "stop",
            message: createAssistantMessage([{ type: "text", text: doneText }]),
          });
        }
        callIndex++;
      });
  ...
  async function collectLastToolExecutionEnd(
    stream: ReturnType<typeof agentLoop>,
  ): Promise<Extract<AgentEvent, { type: "tool_execution_end" }> | undefined> {
    let end: Extract<AgentEvent, { type: "tool_execution_end" }> | undefined;
    for await (const event of stream) {
      if (event.type === "tool_execution_end") {
        end = event;
      }
  ...
  function errorText(end: Extract<AgentEvent, { type: "tool_execution_end" }> | undefined): string | undefined {
    const content = (end?.result as { content?: Array<{ type: string; text?: string }> } | undefined)?.content;
    return content?.find((block) => block.type === "text")?.text;
  }

  it("emits an error tool result instead of executing when the tool is unknown", async () => {
    const stream = agentLoop(
      ...
      undefined,
      toolCallThenDone(
        createAssistantMessage([{ type: "toolCall", id: "tool-1", name: "missing", arguments: {} }], "toolUse"),
      ),
    );
    const end = await collectLastToolExecutionEnd(stream);
    expect(end?.isError).toBe(true);
    expect(errorText(end)).toBe("Tool missing not found");
  });

  it("emits an error tool result when argument validation fails and does not execute", async () => {
    const execute = vi.fn();
    const stream = agentLoop(
      [createUserMessage("run")],
      baseContext([
      ...
      toolCallThenDone(
        createAssistantMessage(
          [{ type: "toolCall", id: "tool-1", name: "echo", arguments: { value: 42 } }],
          "toolUse",
        ),
      ),
    );
    ...
    expect(end?.isError).toBe(true);
    expect(errorText(end)).toContain('Validation failed for tool "echo":');
    expect(errorText(end)).toContain("Received arguments:");
  });

  it("emits an error tool result when prepareArguments throws", async () => {
    const execute = vi.fn();
    const stream = agentLoop(
      [createUserMessage("run")],
      baseContext([
      ...
      undefined,
      toolCallThenDone(
        createAssistantMessage([{ type: "toolCall", id: "tool-1", name: "edit", arguments: {} }], "toolUse"),
      ),
    );
    const end = await collectLastToolExecutionEnd(stream);
    expect(execute).not.toHaveBeenCalled();
    expect(end?.isError).toBe(true);
    expect(errorText(end)).toBe("preparation exploded");
  });

  it("emits the default blocked message when beforeToolCall blocks without a reason", async () => {
    const execute = vi.fn();
    const stream = agentLoop(
      [createUserMessage("run")],
      baseContext([
      ...
      baseConfig({ beforeToolCall: async () => ({ block: true }) }),
      undefined,
      toolCallThenDone(
        createAssistantMessage([{ type: "toolCall", id: "tool-1", name: "work", arguments: {} }], "toolUse"),
      ),
    );
    const end = await collectLastToolExecutionEnd(stream);
    expect(execute).not.toHaveBeenCalled();
    expect(end?.isError).toBe(true);
    expect(errorText(end)).toBe("Tool execution was blocked");
  });

  it("uses the beforeToolCall block reason as the error content", async () => {
    const execute = vi.fn();
    const stream = agentLoop(
      [createUserMessage("run")],
      baseContext([
      ...
      baseConfig({ beforeToolCall: async () => ({ block: true, reason: "denied by policy" }) }),
      undefined,
      toolCallThenDone(
        createAssistantMessage([{ type: "toolCall", id: "tool-1", name: "work", arguments: {} }], "toolUse"),
      ),
    ...
    const end = await collectLastToolExecutionEnd(stream);
    expect(end?.isError).toBe(true);
    expect(errorText(end)).toBe("denied by policy");
  });

  it("encodes a throwing beforeToolCall as an error tool result (hook contract violated)", async () => {
    const execute = vi.fn();
    const stream = agentLoop(
      [createUserMessage("run")],
      baseContext([
    ...
    expect(execute).not.toHaveBeenCalled();
    expect(end?.isError).toBe(true);
    expect(errorText(end)).toBe("review blew up");
  });

  it("applies afterToolCall field overrides without deep merging", async () => {
    const stream = agentLoop(
      [createUserMessage("run")],
      baseContext([
        {
    ...
    expect(end?.isError).toBe(true);
    expect((end?.result as { content: Array<{ type: string; text: string }>; details: unknown }).content).toEqual([
      { type: "text", text: "overridden" },
    ]);
    expect((end?.result as { details: unknown }).details).toEqual({ keep: true });
  });

  it("encodes a throwing afterToolCall as an error tool result", async () => {
    const stream = agentLoop(
      [createUserMessage("run")],
      baseContext([
        {
    ...
    const end = await collectLastToolExecutionEnd(stream);
    expect(end?.isError).toBe(true);
    expect(errorText(end)).toBe("after blew up");
  });
});
...
describe("agentLoopContinue with AgentMessage", () => {
  ...
  it("should continue from existing context without emitting user message events", async () => {
    ...
    expect((messageEndEvents[0] as AgentEvent & { type: "message_end" }).message.role).toBe("assistant");
  });
  ...
  it("should allow custom message types as last message (caller responsibility)", async () => {
    ...
    const config: AgentLoopConfig = {
      ...
      convertToLlm: (messages) => {
        ...
        return messages
          .map((m) => {
            if ((m as unknown as { role: string }).role === "custom") {
              return {
                role: "user" as const,
