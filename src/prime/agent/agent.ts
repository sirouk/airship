/**
 * Port of prime-agent packages/agent/src/agent.ts.
 *
 * Stateful wrapper around the low-level agent loop: owns the transcript,
 * emits lifecycle events, executes tools, and exposes queueing APIs for
 * steering and follow-up messages. Settlement semantics are preserved
 * exactly: the agent stays busy until awaited `agent_end` listeners finish.
 *
 * Dependency change: the run-failure diagnostic uses ../ai/types
 * AssistantMessageDiagnostic ({code, message, detail?}) instead of the
 * upstream pi-ai diagnostics helper ({type, timestamp, error, details}),
 * because that helper is not part of the ported ai surface.
 */
...
/**
 * Record a loop-body failure (a violated hook contract, e.g. a throwing
 * convertToLlm) as a diagnostic on the synthesized failure message so callers
 * can distinguish lifecycle failures from provider errors without parsing
 * message text. Downstream retry policy keys on the stable code string.
 */
function createRunFailureDiagnostic(error: unknown): AssistantMessageDiagnostic {
  return {
    code: "agent_lifecycle_failure",
...
export class Agent {
  ...
  private async handleRunFailure(error: unknown, aborted: boolean): Promise<void> {
    const failureMessage = {
      ...
      diagnostics: aborted ? undefined : [createRunFailureDiagnostic(error)],
      timestamp: Date.now(),
