...
import type { AgentContext, AgentLoopConfig, AgentMessage, AgentTool } from "./types";

/**
 * Port of prime-agent packages/agent/test/agent.test.ts.
 * Upstream pulls catalog models via getModel(); the ported ai layer ships no
 * generated catalog, so the tests use the local createModel() factory - the
 * provider is never invoked in those paths.
 */

function createTextAssistantMessage(text: string) {
  return createAssistantMessage([{ type: "text", text }]);
}

function createToolUseMessage(name: string, id = "calc-1", args: Record<string, unknown> = {}) {
  return createAssistantMessage([{ type: "toolCall", id, name, arguments: args }], "toolUse");
}
...
describe("Agent", () => {
  ...
  it("should await async subscribers before prompt resolves", async () => {
    ...
    const agent = new Agent({
      streamFn: () => {
        ...
        queueMicrotask(() => {
          stream.push({ type: "done", reason: "stop", message: createTextAssistantMessage("ok") });
        });
        return stream;
      },
    });
  ...
  it("should update state with mutators", () => {
    ...
    // Test setTools
    const tools = [{ name: "test", description: "test tool" } as unknown as AgentTool];
    ...
    agent.state.messages.push(newMessage as unknown as AgentMessage);
  ...
  it("should settle when aborting a tool that ignores the abort signal", async () => {
    ...
    const hangingTool: AgentTool<Record<string, never>, Record<string, never>> = {
      name: "hang",
      label: "hang",
  ...
  it("continue() should process queued follow-up messages after an assistant turn", async () => {
    const agent = new Agent({
      streamFn: () => {
        const stream = new MockAssistantStream();
        queueMicrotask(() => {
          stream.push({ type: "done", reason: "stop", message: createAssistantMessage([{ type: "text", text: "Processed" }]) });
        });
        return stream;
      },
    });
  ...
  it("keeps queued message batches atomic in one-at-a-time mode", async () => {
    ...
    agent.state.messages = [createAssistantMessage([{ type: "text", text: "Initial response" }])];
  ...
  it("records a failed turn, emits agent_end, and settles idle when the stream errors", async () => {
    const agent = new Agent({
      streamFn: () => {
        const stream = new MockAssistantStream();
        queueMicrotask(() => {
          const failed = createAssistantMessage([{ type: "text", text: "" }], "error");
          failed.errorMessage = "provider exploded";
