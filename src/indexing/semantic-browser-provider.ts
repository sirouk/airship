...
/**
 * Both embedding engines run on this device: `bootstrap` is the deterministic
 * hash provider that needs no download, `semantic` is the local transformer
 * pack in a worker. The corpus never leaves the machine to be embedded.
 */
 ...
 * stayed on hash vectors until someone found a button. A stored value this
 * build no longer offers reads as no preference, so an old preference cannot
 * pin a mode that does not exist.
 */
